...
const ROLES = [
  { role: 'ADMIN', desc: 'Full access: create/clone tickets, submit for approval, edit, delete, close tickets, Audit Log.' },
  ...
  { role: 'RTB_OWNER', desc: 'RTB Lead. Approve/reject, assign tickets to ADMIN or TECH_LEAD, edit tickets, move status.' },
  { role: 'TECH_LEAD', desc: 'BTB Lead. Owns technical fix, edits tickets, moves status through resolution.' },
  { role: 'READ_ONLY', desc: 'View Dashboard, Tickets, and Knowledge Base only. No create, edit, or approve.' },
];
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-12">
      ...
      <section className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sm:p-8">
        ...
        <ul className="space-y-2 text-slate-700">
          <li><Link to="/dashboard" className="text-emerald-600 hover:underline font-medium">Dashboard</Link> – metrics, charts, quick ticket list</li>
          <li><Link to="/tickets" className="text-emerald-600 hover:underline font-medium">Tickets</Link> – full list, search, filters, export</li>
          <li><Link to="/tickets/create" className="text-emerald-600 hover:underline font-medium">Create Ticket</Link> – new problem (ADMIN)</li>
          <li><Link to="/approvals" className="text-emerald-600 hover:underline font-medium">Approvals</Link> – pending approvals (REVIEWER, APPROVER, RTB_OWNER, ADMIN)</li>
          <li><Link to="/knowledge" className="text-emerald-600 hover:underline font-medium">Knowledge Base</Link> – articles from resolved tickets</li>
          <li><Link to="/audit" className="text-emerald-600 hover:underline font-medium">Audit Log</Link> – who did what (ADMIN only)</li>
        </ul>
      </section>
      ...
      <section className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sm:p-8">
        ...
        <p className="text-slate-700 mb-3">
          Admin creates a ticket (NEW) → Submits for approval → Reviewer, Approver, and RTB Owner review in parallel → Any one approves → ASSIGNED →
          RTB Owner assigns to Admin or Tech Lead → Tech Lead/Admin works on fix (IN_PROGRESS → ROOT_CAUSE → FIX_IN_PROGRESS → RESOLVED) →
          Admin closes (CLOSED). When a ticket is Resolved, a Knowledge Base article is auto-created.
        </p>
      ...
      <section className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sm:p-8">
        ...
        <p className="text-slate-700">
          ...
          root cause, workaround, and permanent fix so you can reuse past solutions. Open <Link to="/knowledge" className="text-emerald-600 hover:underline">Knowledge Base</Link> to browse.
        </p>
      </section>
      ...
      <div className="text-center text-sm text-slate-500">
        Need more detail? Check the full <code className="bg-slate-100 px-1 rounded">docs/USER_GUIDE.md</code> or ask your FAST admin.
      </div>
    </div>
  );
}
